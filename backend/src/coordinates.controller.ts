...
export class CoordinatesController {
  ...
  async handleCron(): Promise<void> {
    ...
    try {
      // MOCKED: Hardcoded current coordinates (bypassing model call)
      const current_coord: Coordinates = {
        lat: 40.015,
        lon: -105.2705,
      };
      ...
      const notificationPromises = predictedPayload.map(async (prediction: PredictionPayload) => {
        const predicted_coordinate: Coordinates = {
          lat: prediction.current_lat,
          lon: prediction.current_long,
        };

        this.log.debug(`Predicted coordinates ${predicted_coordinate.lat}, ${predicted_coordinate.lon}`);
        ...
        if (notifications && notifications.length > 0) {
          const firstNotification = notifications[0] as { title: string; body: string };
          return {
            title: firstNotification.title,
            body: firstNotification.body,
          } as Notification;
  ...
  async runPrediction(@Req() req: Request<HTTPPredictionRequest>, @Res() res: Response): Promise<Response> {
    ...
    try {
      // MOCKED: Skip real API call
      this.log.debug(`Received coordinates lat=${currentCoordinates.lat}, lon=${currentCoordinates.lon}`);
      return res.status(200).json({ data: 'Coordinates received successfully' });
    } catch (err) {
      this.log.error(`Failed to record coordinates: ${err.message}`);
      return res
        .status(500)
